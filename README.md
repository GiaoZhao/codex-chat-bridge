...
/threads    列出最近的 Codex 任务（每页 6 个）
/threads 2  查看第 2 页
/search 支付退款    按关键词检索 Codex 任务
/search 2 支付退款  查看检索结果第 2 页
...
`/threads` 和 `/search` 中 `*` 表示当前任务。`/search` 对任务名称、标题、首条消息、
预览、项目目录和 Thread UUID 执行只读关键词检索；多个关键词需全部匹配，但可以分布在
不同字段。列表中的任务按钮使用完整 Thread UUID 切换，不依赖可能随任务更新而变化的
页码编号。切换成功后会直接读取本地 JSONL，返回最后一条已有 Codex 最终回复的用户消息
及对应最终回复；这段内容不调用模型、不总结、不改写。之后该渠道后续的普通消息都会进入
新选择的任务。
